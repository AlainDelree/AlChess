...
- **Chantier « l'UI reflète l'état réel du système »** `[Les deux]` — Regroupe plusieurs points de la même famille, à concevoir ensemble :
  ...
  - **Griser les moteurs non disponibles** — détecter **au démarrage** quels moteurs sont réellement lançables (lc0/rodent/stockfish présents ET fonctionnels — vérif légère, pas trop lourde). Si un moteur est mort/absent : afficher un message, et **griser ce moteur partout** où il apparaît dans le programme. Nécessite détection back + événement Socket.IO + logique front + i18n.
