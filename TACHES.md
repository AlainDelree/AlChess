...
- **Chantier installeur NSIS compilé (`AlChess_Setup.exe`) — phase 1 (spike) FAITE** `[Windows]` (issue #50) — Décision : créer un second installeur compilé (`.exe` via NSIS) **en parallèle** de `install_alchess.ps1` (jamais touché), pour contourner les policies d'exécution PowerShell (point 2 des retours utilisateur externe, suite de #40/#46/#47/#49). Phase 1 = spike de faisabilité seulement : dossier `installer-exe/` + `alchess_setup.nsi` minimal créés et commités. **Bloquant sur ce poste** : `nsis`/`makensis` non installé et `sudo` non disponible en mode non-interactif → compilation non réalisée ici. À faire manuellement : `sudo apt install -y nsis`, puis `makensis installer-exe/alchess_setup.nsi`, puis lister les plugins (`ls /usr/share/nsis/Plugins/`) pour savoir si Inetc (téléchargement HTTPS) est présent. Phases suivantes (portage réel de la logique de l'installeur) : voir issue #50.
