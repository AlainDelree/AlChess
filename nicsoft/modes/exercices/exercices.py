...
def get_mes_lignes() -> list:
    """Charge les lignes personnelles depuis mes_lignes.json."""
    if not MES_LIGNES_JSON.exists():
        return []
    try:
        import json
        return json.loads(MES_LIGNES_JSON.read_text(encoding="utf-8"))
    except Exception:
        return []
...
class ExerciceSession:
    ...
    def __init__(self, nl_inst, ouverture: dict,
        ...
        engine : EngineManager optionnel — utilisé pour continuer après fin de livre
        """
        ...
        # Mode "ligne perso" : source = mes_lignes.json, pas de livre Polyglot
        # Si "source" est présent dans l'entrée, on utilise les coups d'init comme ligne unique
        self._init_only   = bool(ouverture.get("source", ""))
    ...
    # ── Livre Polyglot ou ligne perso ─────────────────────────────────────────

    def _get_book_moves(self, board: chess.Board) -> list:
        """Retourne les coups du livre pour la position, triés par poids décroissant.
        En mode init_only : retourne un pseudo-entry avec le prochain coup de la ligne."""
        if self._init_only:
            # Le prochain coup de la ligne perso est le coup n° len(board.move_stack)
            n = len(board.move_stack)
            all_line = self.ouverture.get("line", self.ouverture.get("init", []))
            if n < len(all_line):
                try:
                    move = chess.Move.from_uci(all_line[n])
                    if move in board.legal_moves:
                        class _FakeEntry:
                            def __init__(self, m): self.move = m; self.weight = 1
                        return [_FakeEntry(move)]
                except Exception:
                    pass
            return []
        try:
        ...
        while self._running:
            ...
            if not in_book:
                send_event("exercice_end_of_line", {
                    ...
                    "nom":       self.ouverture["nom"],
                    "can_continue": self._engine is not None,
                })
                # Attendre action (rejouer, continuer ou retour)
                result = self._wait_action_end()
                if result == "RETRY":
                    self._restart()
                    continue
                if result == "CONTINUE" and self._engine is not None:
                    self._run_free()
                return
            ...
            if self.board.turn == self.human_color:
                ...
                # Pause en mode virtuel — donne l'impression que le livre réfléchit
                from nicsoft.niclink.virtual_board import VirtualBoard
                if isinstance(self.nl_inst, VirtualBoard):
                    time.sleep(1.0)
    ...
    def _run_free(self) -> None:
        """
        Continue la partie librement après la fin du livre.
        Le moteur joue pour l'adversaire, le joueur humain joue ses coups normalement.
        """
        ...
        while self._running:
            ...
            if self.board.turn == self.human_color:
                ...
                self._send_position()
                move = self._wait_human_move()
                if move is None or not self._running:
                    return
                if move == "RETRY":
                    self._restart()
                    self.run()
                ...
                # Tour moteur
                try:
                    adv_move = self._engine.get_move(self.board, think_time=1.0)
                    if adv_move is None:
                        return
                    uci = adv_move.uci()
                    san = self.board.san(adv_move)
                    ...
                    logger.error(f"_run_free moteur : {e}")
                    return

    def _wait_action_end(self) -> str:
        """Attend une action de fin (retry, continue ou back). Retourne 'RETRY', 'CONTINUE' ou None."""
        while self._running:
...
def common_init_moves(lignes: list[dict]) -> list[str]:
    """
    Calcule les coups d'init communs à toutes les lignes sélectionnées.
    Retourne le préfixe commun le plus long.
    """
    if not lignes:
        return []
    inits = [l.get("init", []) for l in lignes]
    common = []
    for moves in zip(*inits):
        if len(set(moves)) == 1:
...
def build_virtual_book(lignes: list[dict]) -> dict[str, list[str]]:
    """
    Construit un livre virtuel depuis une liste de lignes PGN.
    Retourne un dict {fen_key: [uci_move, ...]} où fen_key est le FEN
    de la position (sans horloge), et les valeurs sont les coups possibles.
    """
    import chess
    book = {}
    for ligne in lignes:
        moves = ligne.get("line", ligne.get("init", []))
        board = chess.Board()
...
class DrillSession(ExerciceSession):
    """
    Session de drill multi-lignes.
    Utilise un livre virtuel construit depuis les lignes sélectionnées.
    """

    def __init__(self, nl_inst, lignes: list[dict],
                 human_color: str = "white",
                 engine=None) -> None:
        # Construire une ouverture synthétique pour ExerciceSession
        init = common_init_moves(lignes)
        camp = lignes[0].get("camp_suggere", human_color) if lignes else human_color
        ouverture = {
            "id":           "drill_multi",
            "nom":          f"Drill ({len(lignes)} lignes)",
            "init":         init,
            "line":         [],
            "camp_suggere": camp,
            "book":         "",
        ...
        super().__init__(nl_inst, ouverture, human_color, variete=1, engine=engine)
        self._virtual_book = build_virtual_book(lignes)
        self._init_only    = True  # désactive le livre Polyglot

    def _get_book_moves(self, board) -> list:
        """Cherche dans le livre virtuel les coups possibles pour la position."""
        key = board.board_fen() + ("w" if board.turn == chess.WHITE else "b")
        moves_uci = self._virtual_book.get(key, [])
        if not moves_uci:
            return []
        ...
        for uci in moves_uci:
            try:
                move = chess.Move.from_uci(uci)
                if move in board.legal_moves:
                    result.append(_FakeEntry(move))
            except Exception:
                pass
        return result

    def _pick_book_move(self, board) -> chess.Move | None:
        """Choisit aléatoirement parmi les coups du livre virtuel."""
        import random
        entries = self._get_book_moves(board)
        if not entries:
            return None
