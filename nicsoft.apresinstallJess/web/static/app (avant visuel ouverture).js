...
function _exIsRoot(o) {
  // Racine = eco contient un tiret (plage) ex: "C00-C19"
  return (o.eco || "").includes("-");
}

function _exEcoInRange(eco, range) {
  // Vérifie si un code ECO simple tombe dans une plage "C00-C19"
  if (!range.includes("-")) return eco.startsWith(range);
  const [lo, hi] = range.split("-");
  return lo <= eco && eco <= hi;
}

function _exFindParent(variant, roots) {
  // Cherche l'ouverture racine dont la plage ECO contient le code de la variante
  return roots.find(r => _exEcoInRange(variant.eco, r.eco)) || null;
}

function _exBuildCard(o, color, isVariant) {
  const card = document.createElement("div");
  const w = isVariant ? "220px" : "250px";
  card.style.cssText = `background:#16213e; border:2px solid ${color}44; border-radius:8px; padding:${isVariant ? "10px 12px" : "12px 14px"}; width:${w}; cursor:pointer; transition:border-color 0.15s; box-sizing:border-box;`;
  card.onmouseover = () => card.style.borderColor = color;
  card.onmouseout  = () => card.style.borderColor = `${color}44`;
  const miniBoard = _exMiniBoardSvg(o.init || []);
  const ecoSize   = isVariant ? "0.68rem" : "0.72rem";
  const nomSize   = isVariant ? "0.82rem" : "0.92rem";
  card.innerHTML = `
    <div style="font-size:${ecoSize}; color:${color}; margin-bottom:3px;">${o.eco}</div>
    <div style="font-size:${nomSize}; font-weight:bold; color:#e0e0e0; margin-bottom:5px;">${o.nom}</div>
    <div style="font-size:0.72rem; color:#778; line-height:1.4; margin-bottom:8px;">${o.desc}</div>
    <div style="display:flex; justify-content:center;">${miniBoard}</div>
  `;
  card.onclick = () => exLancer(o.id);
  return card;
}
...
function exRenderOuvertures(ouvertures) {
  ...
  // Séparer racines et variantes
  const roots    = filtered.filter(o => _exIsRoot(o));
  const variants = filtered.filter(o => !_exIsRoot(o));

  // Associer variantes à leur racine
  const variantsByRoot = {};
  const orphans = [];
  for (const v of variants) {
    const parent = _exFindParent(v, roots);
    if (parent) {
      if (!variantsByRoot[parent.id]) variantsByRoot[parent.id] = [];
      variantsByRoot[parent.id].push(v);
    } else {
      orphans.push(v);  // pas de racine → traiter comme racine
    }
  }

  // Grouper racines (+ orphelins) par famille
  ...
  for (const [fam, items] of Object.entries(groups)) {
    ...
    for (const o of items) {
      ...
      rootBlock.style.cssText = "margin-bottom:12px;";

      if (hasVariants) {
        // Carte racine cliquable pour expand/collapse variantes
        const rootHeader = document.createElement("div");
        const rootOpen   = sessionStorage.getItem(rootKey) !== "closed";
        rootHeader.style.cssText = `display:flex; align-items:center; gap:10px; padding:6px 12px; background:${color}11; border-left:3px solid ${color}88; border-radius:4px; cursor:pointer; user-select:none; margin-bottom:${rootOpen ? "8px" : "0"};`;
        rootHeader.innerHTML = `
          <div style="display:flex; align-items:center; gap:8px; flex:1;">
            ${_exMiniBoardSvg(o.init || [])}
            <div>
              <div style="font-size:0.72rem; color:${color}88; margin-bottom:2px;">${o.eco}</div>
              <div style="font-size:0.9rem; font-weight:bold; color:#e0e0e0;">${o.nom}</div>
              <div style="font-size:0.72rem; color:#556; margin-top:2px;">${childVariants.length} variante${childVariants.length > 1 ? "s" : ""}</div>
            </div>
          </div>
          <span style="color:${color}; font-size:1rem;">${rootOpen ? "▲" : "▼"}</span>
        `;

        const varGrid = document.createElement("div");
        varGrid.style.cssText = `display:${rootOpen ? "flex" : "none"}; flex-wrap:wrap; gap:10px; padding:0 4px 8px 16px;`;

        rootHeader.onclick = () => {
          const open = varGrid.style.display === "none";
          varGrid.style.display = open ? "flex" : "none";
          rootHeader.style.marginBottom = open ? "8px" : "0";
          rootHeader.querySelector("span:last-child").textContent = open ? "▲" : "▼";
          sessionStorage.setItem(rootKey, open ? "open" : "closed");
        };

        // Jouer la racine directement via double-clic
        rootHeader.ondblclick = (e) => { e.stopPropagation(); exLancer(o.id); };
        rootHeader.title = "Double-clic pour jouer cette ligne, clic simple pour voir les variantes";

        for (const v of childVariants) {
          varGrid.appendChild(_exBuildCard(v, color, true));
        }

        rootBlock.appendChild(rootHeader);
        rootBlock.appendChild(varGrid);
      } else {
        // Pas de variantes → carte normale
        rootBlock.appendChild(_exBuildCard(o, color, false));
      }

      famContent.appendChild(rootBlock);
    }
